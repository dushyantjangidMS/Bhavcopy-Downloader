"""
Bhavcopy Analyzer — Streamlit Application
==========================================
A production-ready app to download, analyze, and display NSE & BSE
derivative Bhavcopy data with automatic expiry-date calculation.
...
st.markdown(
    ...
    .metric-card .value.success { color: #4ade80; }
    .metric-card .value.info    { color: #60a5fa; }
...
def _init_state():
    ...
    defaults = {
        ...
        "bse_expiry": None,
        "trade_date": None,
        "global_skip": None,      # reason if entire day is blocked
        "nse_skip": None,         # reason if NSE specifically skipped
        "bse_skip": None,         # reason if BSE specifically skipped
        "holidays": DEFAULT_NSE_HOLIDAYS_2026.copy(),
        "active_mode": None,
        "bulk_zip": None,         # bytes for bulk ZIP download
        "bulk_zip_name": None,    # filename for bulk ZIP
    }
...
def _clear_results():
    """Reset all result / output state so no stale data lingers."""
    st.session_state.logs = []
    st.session_state.nse_df = None
    st.session_state.bse_df = None
    st.session_state.nse_expiry = None
    st.session_state.bse_expiry = None
    st.session_state.trade_date = None
    st.session_state.global_skip = None
    st.session_state.nse_skip = None
    st.session_state.bse_skip = None
    st.session_state.bulk_zip = None
    st.session_state.bulk_zip_name = None
...
def render_logs():
    ...
    lines = []
    for ts, level, msg in st.session_state.logs:
        css = css_map.get(level, "log-info")
        icon = {"success": "✅", "error": "❌", "info": "ℹ️", "warn": "⚠️"}.get(
            level, "•"
        )
...
def process_date(trade_date: datetime, holidays: list[str]):
    """
    For a given trade date:
      1. Validate per-exchange (NSE and BSE independently)
      2. Fetch only the exchanges that are valid
      3. Store skip reasons per exchange
    """
    ...
    # ── Always compute expiry dates (for display) ────────────────────
    nse_exp = compute_nse_expiry(trade_date, holidays)
    bse_exp = compute_bse_expiry(trade_date, holidays)
    st.session_state.nse_expiry = nse_exp
    st.session_state.bse_expiry = bse_exp
    ...
    if not v["is_trading_day"]:
        # Weekend or holiday — skip both
        st.session_state.global_skip = v["global_reason"]
        add_log(f"⛔ SKIPPED (both): {v['global_reason']}", "warn")
        return

    # ── NSE Bhavcopy (independent) ───────────────────────────────────
    if v["nse_ok"]:
        ...
        st.session_state.nse_skip = v["nse_reason"]
        add_log(f"NSE: ⛔ {v['nse_reason']}", "warn")

    # ── BSE Bhavcopy (independent) ───────────────────────────────────
    if v["bse_ok"]:
        ...
        for line in bse_log.strip().split("\n"):
            lvl = "success" if "✅" in line else ("error" if "❌" in line else "info")
            add_log(line, lvl)
        st.session_state.bse_df = bse_df
    else:
        st.session_state.bse_skip = v["bse_reason"]
...
# ──────────────────────────────────────────────────────────────────────
def render_results():
    """Display metric cards and download-only buttons (no previews)."""
    if st.session_state.trade_date is None:
        return
    ...
    with c1:
        st.markdown(
            metric_card(
                "Trade Date",
                format_date_display(st.session_state.trade_date),
                "info",
            ),
            unsafe_allow_html=True,
        )
    with c2:
        st.markdown(
            metric_card(
                "NSE Expiry (Tue)",
                format_date_display(st.session_state.nse_expiry)
                if st.session_state.nse_expiry
                else "—",
                "success",
            ),
            unsafe_allow_html=True,
    ...
    with c3:
        st.markdown(
            metric_card(
                "BSE Expiry (Thu)",
                format_date_display(st.session_state.bse_expiry)
                if st.session_state.bse_expiry
                else "—",
                "warn",
            ),
            unsafe_allow_html=True,
    ...
    # ── If the entire day was blocked (weekend/holiday) ───────────────
    if st.session_state.global_skip:
        st.warning(f"🚫 {st.session_state.global_skip}")
        return

    # ── Per-exchange download buttons / skip messages ─────────────────
    dl1, dl2 = st.columns(2)

    with dl1:
        if st.session_state.nse_skip:
            st.warning(f"🚫 NSE: {st.session_state.nse_skip}")
        elif st.session_state.nse_df is not None:
            df = st.session_state.nse_df
            csv_buf = df.to_csv(index=False).encode("utf-8")
            st.download_button(
                f"⬇️ Download NSE Bhavcopy ({len(df)} rows)",
                csv_buf,
                file_name=f"NSE_Bhavcopy_{format_date_display(st.session_state.trade_date)}.csv",
                mime="text/csv",
    ...
    with dl2:
        if st.session_state.bse_skip:
            st.warning(f"🚫 BSE: {st.session_state.bse_skip}")
        elif st.session_state.bse_df is not None:
            df = st.session_state.bse_df
            csv_buf = df.to_csv(index=False).encode("utf-8")
            st.download_button(
                f"⬇️ Download BSE Bhavcopy ({len(df)} rows)",
                csv_buf,
                file_name=f"BSE_Bhavcopy_{format_date_display(st.session_state.trade_date)}.csv",
                mime="text/csv",
                use_container_width=True,
            )
        else:
...
def main():
        ...
        if uploaded_files:
            if st.button("🚀 Process Uploaded Files", type="primary", use_container_width=True):
                ...
                for idx, uf in enumerate(uploaded_files, 1):
                    progress.progress(
                        idx / total,
                        text=f"Processing {uf.name} ({idx}/{total})…",
                    )
    ...
    # ─────────────────────────────────────────────────────────────────
        ...
        selected = st.date_input("Pick a date", value=datetime.today())
    ...
    # ─────────────────────────────────────────────────────────────────
    # MODE 3: Date range (bulk) → ZIP download
    # ─────────────────────────────────────────────────────────────────
        ...
        if start_date > end_date:
            ...
            # Pre-classify each day using per-exchange validation
            processable_days = []   # days where at least one exchange is valid
            fully_skipped = []      # days where both are blocked (weekend/holiday)
            for day in all_days:
            ...
            if st.button(
                ...
                # Log fully skipped days
                for day, reason in fully_skipped:
                    add_log(
                        f"⛔ {format_date_display(day)}: {reason} — skipped",
                        "warn",
                    )
                ...
                if not processable_days:
                    ...
                    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                        for idx, (day, v) in enumerate(processable_days, 1):
                            ...
                            # Compute expiry dates (for logging)
                            ...
                            # Fetch NSE (only if not NSE expiry)
                            if v["nse_ok"]:
                                ...
                                if nse_df is not None:
                                    zf.writestr(
                                        f"NSE_bhavcopy_{day_label}.csv",
                                        nse_df.to_csv(index=False),
                                    )
                                    file_count += 1
                            else:
                                add_log(f"NSE: ⛔ {v['nse_reason']}", "warn")

                            # Fetch BSE (only if not BSE expiry)
                            if v["bse_ok"]:
                                ...
                                if bse_df is not None:
                                    zf.writestr(
                                        f"BSE_bhavcopy_{day_label}.csv",
                                        bse_df.to_csv(index=False),
                                    )
                                    file_count += 1
                            else:
                                add_log(f"BSE: ⛔ {v['bse_reason']}", "warn")
                    ...
                    if file_count > 0:
                        zip_buffer.seek(0)
                        range_label = (
                            f"{start_date.strftime('%d%b%Y')}_to_{end_date.strftime('%d%b%Y')}"
                        ).lower()
                        st.session_state.bulk_zip = zip_buffer.getvalue()
                        st.session_state.bulk_zip_name = f"Bhavcopy_{range_label}.zip"

                        # Store the last day's expiry info for result cards
                        last_day = processable_days[-1][0]
                        st.session_state.trade_date = last_day
                        st.session_state.nse_expiry = compute_nse_expiry(last_day, holidays)
                        st.session_state.bse_expiry = compute_bse_expiry(last_day, holidays)
                        st.session_state.global_skip = None

    st.markdown("---")

    # ── Results ──────────────────────────────────────────────────────
    # For file-upload and single-date modes: show metric cards + downloads
    if mode != "📆 Select Date Range (Bulk)":
        render_results()
    else:
        # Bulk mode: show metric cards (last valid date) + ZIP download
        if st.session_state.trade_date is not None:
            st.markdown('<div class="section-title">📋 Results</div>', unsafe_allow_html=True)
